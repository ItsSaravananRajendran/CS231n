...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    d_tanh = (1 - np.square(next_h)) * dnext_h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for I in range(T):
        X = x[:,I,:]
        h[:,I+1,:] ,cache[I] = rnn_step_forward(X, h[:,I,:], Wx, Wh, b)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return h[:,1:,:], cache
...
def rnn_backward(dh, cache):
    ...
    for I in reversed(range(T)):
        curr = dh[:,I,:] + dprev_h
        dx_, dprev_h , dWx_, dWh_, db_ = rnn_step_backward(curr, cache[I])
        dx[:,I,:] += dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
